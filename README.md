...
Exact Decimal amounts stay decimal strings in JSON. Currency summaries remain
separate because no versioned conversion policy is configured.
...
results. The only demonstration facts are synthetic. This slice does not
...
`tests.test_outcomes_postgresql`. The candidate-bound browser report and
1440×900 / 390×844 screenshots are under
`evidence/u2/chg-205-u2.2-o7.1/`; the recorded local PostgreSQL version is
reported in that evidence rather than inferred to be the CI PostgreSQL 18.x
lane.
