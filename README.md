...
The self-check reports package identity, runtime/config identity, the pinned framework identity and the explicit `NOT_IMPLEMENTED`/`NOT_RUN` state of F02/F03/F04/F05. It does not claim historical source identity, byte identity, algorithm equivalence or historical-test equivalence.
...
The runner verifies preserved historical probe identities, runs the canonical self-check, and reports F02/F03/F04/F05 as `NOT_IMPLEMENTED`/`NOT_RUN` until those APIs are implemented. It does not fabricate PASS and does not block the canonical repository baseline. The old staged-source behavior remains only behind the explicit `--legacy-source` compatibility option for historical fixture coverage.
