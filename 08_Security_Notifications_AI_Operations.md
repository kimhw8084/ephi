...
The O8.1 current-authorization boundary is a reusable application authority, not a company identity implementation. Each protected command/receipt replay, Attention source/query/count/snapshot operation, current or historical Episode read, and artifact metadata/byte/publish-precondition operation re-resolves current authorization before lookup or disclosure. It compares the presented subject and both freshness revisions, then requires the requested scope and capability on the current authority without silently upgrading a stale presented Principal. Resolver failure, subject/revision mismatch and grant removal are bounded fail-closed permission outcomes. Development/test composition may use the explicit environment-backed identity provider; non-development composition has no permissive fallback. Real company `IdentityResolver`, session transport, CSRF/WebSocket-origin controls, upload/export/sanitization and deployment qualification remain pending O8/G07 work.
