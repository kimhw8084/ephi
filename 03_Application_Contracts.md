...
The canonical application now uses one storage-neutral `CurrentAuthorizationAuthority` at this boundary. It resolves current server-side authorization for the presented subject, requires exact subject, `auth_session_revision` and `security_revision` equality, and checks the requested exact scope and capability against both the presented and current grants. A mismatch or unavailable resolver fails closed with a bounded permission error; the authority never replaces a stale presented `Principal`. Consequential command execution, Attention source/query/count/snapshot operations, current and historical Episode reads, and artifact metadata/byte/publish-precondition disclosure inject this authority before protected lookup. Development/test composition binds it to the existing operation-time environment identity provider; production remains fail closed until an approved company identity/current-authorization adapter is bound.

Retained query snapshots remain bound to their original subject, exact scope, cursor integrity material and effective security revision. A fresh authorized principal after a revision change must start a new query; an old snapshot/cursor cannot continue even when the new grants are equivalent. Browser session transport, CSRF/WebSocket-origin qualification, real company identity/session binding, artifact upload/download transport, export sanitization and deployment controls remain later O8/G07 obligations.
