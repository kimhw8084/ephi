...
These are implementation bindings and evidence obligations, not unanswered product architecture questions. The canonical Git-native W0 package baseline is now available; W1 and behavioral qualification proceed against `src/ephi` when authorized. Historical archive identity remains reference-only provenance and is not a prerequisite for repository execution. Repository corrections and their gate mappings are recorded in [13_Package_Review.md](13_Package_Review.md).
