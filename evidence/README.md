...
[review/w0_integrity_regression_contract.json](review/w0_integrity_regression_contract.json) is the CHG-109 contract for F02/F03/F04/F05 regression evaluation. Its historical observations remain reference-only; the default runner targets `src/ephi` and reports unimplemented behavior as `NOT_IMPLEMENTED`/`NOT_RUN`. The optional historical source-bound runner's fresh output belongs under ignored `artifacts/` runtime output.
