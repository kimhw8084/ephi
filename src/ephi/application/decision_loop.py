...
DECISION_LOOP_AGGREGATE_TYPE = "ephi_decision_loop"
...
def _state_object(current: Mapping[str, Any], episode_id: str) -> dict[str, Any]:
    ...
    if state.get("schema_version") != DECISION_LOOP_SCHEMA_VERSION or state.get("episode_id") != episode_id:
        raise CoherentReadConflictError("decision-loop aggregate has an unsupported identity or schema")
    if not isinstance(state.get("cycles"), list) or not state["cycles"]:
        raise CoherentReadConflictError("decision-loop aggregate has no durable cycle history")
    return state
...
class DecisionLoopSnapshot:
    """One coherent current read with separate workflow authorities visible."""
    ...
    def active_cycle_id(self) -> str:
        return str(self.state["active_cycle_id"])

    @property
    def workflow_state(self) -> dict[str, Any]:
        cycle = _active_cycle(self.state)
        return {
            "engineering_work_state": self.state["engineering_work_state"],
    ...
    def closure_state(self) -> dict[str, Any]:
        ...
        return {
            ...
            "last_closure_identity": self.state.get("last_closure_identity"),
        }
...
class DecisionLoopCommandService:
    ...
    def _execute(
        ...
        *,
        ...
        return self.executor.execute(
            ...
            aggregate_type=DECISION_LOOP_AGGREGATE_TYPE,
    ...
    def create_episode(
        ...
        *,
        ...
        episode_id = _identity(episode_id, "episode_id")
        cycle_id = _identity(cycle_id, "cycle_id")
        revisions = _revision_payload(context, create=True)
        payload = {"episode_id": episode_id, "cycle_id": cycle_id, **revisions}

        def effect(_current: Mapping[str, Any], domain: Mapping[str, Any]) -> Mapping[str, Any]:
            return _new_state(episode_id, str(domain["cycle_id"]), domain["viewed_revisions"])

        return self._execute(
            context,
            episode_id,
            command_type="CreateDecisionLoopEpisode",
            payload=payload,
    ...
    initialize_episode = create_episode
    ...
    def request_check(
        ...
        *,
        ...
        def effect(current: Mapping[str, Any], domain: Mapping[str, Any]) -> Mapping[str, Any]:
            ...
            state["last_viewed_revisions"] = dict(domain["viewed_revisions"])
            return state
        ...
        *,
        ...
        def effect(current: Mapping[str, Any], domain: Mapping[str, Any]) -> Mapping[str, Any]:
            ...
            if eligibility.contradictory:
                ...
                if eligibility.eligible and submitted is not None:
        ...
        *,
        ...
        def effect(current: Mapping[str, Any], domain: Mapping[str, Any]) -> Mapping[str, Any]:
            ...
            if domain["disposition"] == ClosureDisposition.CONFIRMED_ISSUE.value:
                if not any(plan.get("state") == RecoveryPlanState.PASS.value for plan in plans.values()):
                    raise InvalidTransitionError("confirmed-issue ordinary closure requires technical recovery PASS")
            ...
            cycle["status"] = "CLOSED"
            state["engineering_work_state"] = "CLOSED"
            state["last_closure_identity"] = closure_identity
            state["last_viewed_revisions"] = dict(domain["viewed_revisions"])
            return state
    ...
    def reopen_episode(
        ...
        *,
        ...
        def effect(current: Mapping[str, Any], domain: Mapping[str, Any]) -> Mapping[str, Any]:
            ...
            cycle_version = max(int(cycle.get("cycle_version", 0)) for cycle in state["cycles"]) + 1
            new_cycle_id = f"{episode_id}:cycle-{cycle_version}"
            while any(cycle.get("cycle_id") == new_cycle_id for cycle in state["cycles"]):
                cycle_version += 1
                new_cycle_id = f"{episode_id}:cycle-{cycle_version}"
            reopened_at = _now()
            state["reopen_history"].append({
            ...
            state["cycles"].append(_empty_cycle(new_cycle_id, cycle_version, reopened_at))
            state["active_cycle_id"] = new_cycle_id
            state["engineering_work_state"] = "OPEN"
            state["last_viewed_revisions"] = dict(domain["viewed_revisions"])
            return state
    ...
    def get_decision_loop(self, principal: Principal, scope: AccessScope, episode_id: str) -> DecisionLoopSnapshot:
        ...
        aggregate = self.store.get_aggregate(scope, DECISION_LOOP_AGGREGATE_TYPE, episode_id)
        if aggregate is None:
            raise AggregateNotFoundError("decision-loop Episode is not available in the requested scope")
        state = _state_object(aggregate.state, episode_id)
        cycle = _active_cycle(state)
        if state["engineering_work_state"] == "CLOSED" and cycle["status"] != "CLOSED":
            raise CoherentReadConflictError("engineering work state and cycle status disagree")
        if state["engineering_work_state"] == "OPEN" and cycle["status"] != "OPEN":
            raise CoherentReadConflictError("engineering work state and cycle status disagree")
        revisions = state.get("last_viewed_revisions")
        ...
        if revisions is not None:
            try:
                revision_vector = RevisionVector(
                    ...
                    int(revisions["workflow_version"]),
